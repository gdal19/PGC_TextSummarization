...
def run_textrank(data: list[dict], sentences_count: int = 3) -> list[str]:
    """Generates extractive summaries using TextRank.
    ...
    """
    print(
        f"Running TextRank model (extracting {sentences_count} sentences)..."
    )
    ...
    for item in tqdm.tqdm(data, desc="Running TextRank"):
        text = item["source"]

        # Parse text into documents and sentences
        parser = PlaintextParser.from_string(text, Tokenizer("english"))

        # TextRank builds a graph of sentences and runs PageRank-like algorithm
...
if __name__ == "__main__":
    ...
    results = run_textrank(test_data)

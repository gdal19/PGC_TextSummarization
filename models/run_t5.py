...
def run_t5(data: list[dict]) -> list[str]:
    """Generates summaries using t5-small.
    ...
    for item in tqdm.tqdm(data, desc=f"Running {model_name} on CPU"):
        # T5 expects a task prefix: 'summarize: ' before the text
        text = "summarize: " + item["source"]

        # Explicitly truncate input to T5's standard max position embedding size (512)
        inputs = tokenizer(
